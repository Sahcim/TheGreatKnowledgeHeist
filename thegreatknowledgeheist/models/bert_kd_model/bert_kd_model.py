...
class BertKDModel(BaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_accuracy", self.accuracy(student_outputs["logits"], batch["labels"])
        )
        self.log("train_f1", self.f1_score(student_outputs["logits"], batch["labels"]))
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "val_accuracy",
            self.accuracy(student_outputs["logits"], batch["labels"]),
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "val_f1",
            self.f1_score(student_outputs["logits"], batch["labels"]),
            on_step=False,
            on_epoch=True,
        )
        return loss
