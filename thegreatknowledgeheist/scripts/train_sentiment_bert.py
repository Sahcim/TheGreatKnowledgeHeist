...
PROJECT_ROOT = Path('/home/maria/Documents/TheGreatKnowledgeHeist/thegreatknowledgeheist')
...
def prepare_dataloaders(dataset, tokenizer):
    ...
    for type in ['train', 'test']:
        data = dataset[type]
        encoded_data = data.map(lambda row: tokenizer(row['content'], padding=True), batched=True)
        encoded_data = encoded_data.rename_column("label", "labels")
        encoded_data.set_format(type='torch', columns=['input_ids', 'token_type_ids', 'attention_mask', 'labels'])
        dataloaders[type] = torch.utils.data.DataLoader(encoded_data, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS)
    return dataloaders
...
def train_model(model, dataloaders):
    ...
    trainer = Trainer(logger=WandbLogger(save_dir=str(PROJECT_ROOT / 'logs'),
...
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', padding="max_length",
                                          return_tensors='pt', max_length=MAX_SENTENCE_LENGTH)
